...
function paintForDomain(domain: WimdDomain): maplibregl.DataDrivenPropertyValueSpecification<string> {
  // Normalize 'deprivation' from URL parameter down to 'overall' property inside the data structure
...
export function MapView({
  ...
  // ── 5. Camera positioning side panel sync ─────────────────────────────
  useEffect(() => {
    const map = mapRef.current;
    if (!map) return;
...
function MapLegend({ wimdDomain }: { wimdDomain: WimdDomain }) {
  ...
  // Normalize key values for proper label lookups
